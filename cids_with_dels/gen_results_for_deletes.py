...
def get_mod_file_info(pydriller_repo,cid):
    ...
    for mod in blame_commit.modifications:
        if mod.old_path == mod.new_path and '.c' in mod.old_path:
            ...
            for deleted in mod.diff_parsed['deleted']:
                mod_file_info[mod.old_path][0].append(deleted[0])
            for added in mod.diff_parsed['added']:
                mod_file_info[mod.old_path][1].append(added[0])
...
def process_cid(repo,pydriller_repo,cid,save_dir):
    for mod_file,mod_ls in get_mod_file_info(pydriller_repo,cid).items():
        ...
        info_before['mod_lines'] = mod_ls[0]
...
def match_path(repo,cid,path_results):
    ...
    for path in path_results:
        if len(path['line_strs'])>=2:
            # print(f'begin to check path:{path["line_strs"]}')
...
repo = Repo('/data1/lingfengbao/AOSP/tlx/symbolic_szz/repo/linux')
pydriller_repo = PyDrillerGitRepo('/data1/lingfengbao/AOSP/tlx/symbolic_szz/repo/linux')

bic_cid = get_bic_cids(repo,pydriller_repo,'0a5a98863c9debc02387b3d23c46d187756f5e2b')
